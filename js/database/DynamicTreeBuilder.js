    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
        ...
        this.toHtml = function() {
            ...
            if (this.isFolder) {
                ...
                s += '<img id="?-node" src="?" width="18" height="36" alt="" />'.format(this.id, nodeIcon);
            ...
            if (this.isDoc) {
                s += '<img src="?" width="18" height="36" alt="" /><img src="?" class="doc-icon"/>'.format((this.isLast() ? self.img.leafEnd : self.img.leaf), self.img.doc);
                s += '<span id="?-text" class="text?" onclick="?.textClick(\'?\')">?</span>'.format(this.id, (self.active == this.id ? '-active' : ''), self.id, this.id, this.text);
            }
            s += '</div>';
            return s;
